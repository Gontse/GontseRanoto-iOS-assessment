...
final class QuestionCardView: UIView {
  ...
  private func addOption(with text: String, addSeperator: Bool = true, setSelected: Bool, index: Int) {
    guard let optionView = SelectableAwnswerView.loadView() else { return }
    optionView.setUp(with: text, delegate: self, index: index)
  ...
  private func createAndAddSeperator() {
    ...
    seperatorView.backgroundColor = .orange
...
extension QuestionCardView: SelectionViewDelegate {
  func didSelect(selectionview: SelectableAwnswerView) {
    guard let index = selectedIndex,
          let selectedView = optionsStackView.arrangedSubviews[index] as? SelectableAwnswerView else { return }
    selectedView.deselect()
    selectedIndex = selectionview.index
